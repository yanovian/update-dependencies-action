...
| `commands` | (empty) | Commands to run after updating, one per line, in order. Every command must exit 0 or no pull request is created. Leave empty to skip straight to the pull request once dependencies are updated. |
...
| `commands-passed` | `true` if every command in `commands` exited successfully. |
